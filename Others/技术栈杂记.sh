...
CNN叫卷积神经网络,是ANN的一种,主要用于图像和语音识别。它源于一个日本人提出的Receptive Field的概念,大概的意思是说,我们看东西或者听东西,总是聚焦在其中几个局部上的,可以从一个复杂的图像或者语音数据上提取中一些小的区域,通过卷积运算进行特征的提取,然后经过多层滤波,变成一个简单的向量,最后输入到一个普通的ANN中,提取出其中的模式。
CNN的主要进步是用这种方法可以大幅降低输入向量的规模（否则想想一张照片里面有多少个点,这组成一个多大的向量啊）,CNN的前端的分层通过一个C层（卷积层,Convolution,用于特征提取）一个S层（池化层,Pooling,用于计算局部平均）多次叠加,降低图像的向量维度,之后输入到神经网络中,判断这个图是什么。比如你可以给网络1万张猫脸,让网络学会猫的模型。以后你再给网络一张照片,网络就可以知道这是猫还是别的什么东西了。AlphaGo的基本原理也是这样的。
}
...
http://www.cnblogs.com/qieerbushejinshikelou/p/3961217.html
http://www.cnblogs.com/sivkun/p/7421904.html
...
其中套接字选项中还有地址和端口重用的选项 SO_REUSEADDR 和 SO_REUSEPORT 这两个选项就是为了避免server 重启时 端口忙的问题。
}

# 负载均衡
{
http://www.cnblogs.com/kevingrace/p/6137881.html #linux负载均衡总结性说明（四层负载/七层负载）
http://www.cnblogs.com/kevingrace/p/8290452.html
#负载均衡有硬件和软件两种.
1:  硬件层的比较牛逼,将4-7层负载均衡功能做到一个硬件里面,如F5,梭子鱼.
2:  目前主流的软件负载均衡分为四层和七层
LVS属于四层负载均衡,工作在tcp/ip协议栈上,通过修改网络包的ip地址和端口来转发, 由于效率比七层高,一般放在架构的前端.
七层的负载均衡有nginx, haproxy, apache等, 工作在应用层,因此可以将HTTP请求等应用数据发送到具体的应用服务器,如将图片请求转发到特定的服务器上,总之可以做到更智能的负载均衡,这些功能在四层负载均衡上不好实现,一般放在架构的后面位置,布置在应用服务器前面.七层负载均衡主要识别HTTP相关ul,head,cookie,session等等同时安全防护减少直接攻击上有重要意义
...
3）四层负载均衡（tcp）
在三次负载均衡的基础上,用ip+port接收请求,再转发到对应的机器。相关协议：TCP/UDP等
4）七层负载均衡（http）
根据虚拟的url或IP,主机名接收请求,再转向相应的处理服务器,相关协议：HTTP/HTTPS,常用代理Nginx

Nginx的upstream做基于http/https端口的7层负载均衡,由于Nginx老版本不支持tcp协议,所以基于tcp/udp端口的四层负载均衡一般用LVS或Haproxy来做。
Nginx从1.9.0版本开始,新增加了一个stream模块,用来实现四层协议的转发、代理或者负载均衡等
...
集合数据结构一般都有这么一个方法：contains。其作用就是判断给定的元素是否存在集合中,这是一个常用的方法。
其最简单的内部实现即遍历集合内的元素,一个个的判断是否与给定元素相等。为了更高效点我们甚至可以采用“更好的（好是相对的）”算法实现。比如如果该集合是已经排序的,那么我们用二分查找来实现contains肯定更好。但是,如果集合的数据量庞大到一定程度,大部分我们熟知的算法不再有什么用了。即使可以使用,但是机器内存也不允许。
...
3.Bloom Filter无法从Bloom Filter集合中删除一个元素。

Bloom Filter的特征我们可以看到不是所有的场景都可以用的,只有在一些能容许少量的误报的情况下使用才行。该算法用很低的误报率却换来了大量的存储空间.
...
# Bit-map算法
{
#将要排序的数的大小,映射为足够长度的bit组中的位置大小,这样位置就是顺序
用一个bit位来标记某个元素对应的Value,而Key即是该元素,由于采用了Bit为单位来存储数据,在存储空间方面可以大大节省。

假设要对0-7内的5个元素(4,7,2,5,3)排序（假设没有重复）,就可采用Bit-map的方法来达到排序的目的。要表示8个数,只需要8个Bit（1Bytes）,首先我们开辟1Byte的空间,将这些空间的所有Bit位都置为0
然后遍历这5个元素,首先第一个元素是4,那么就把4对应的位置为1（可以这样操作 'p+(i/8)|(0×01<<(i%8))',当然了这里的操作涉及到Big-ending和Little-ending的情况,这里默认为Big-ending）,因为是从零开始的,所以要把第五位设置为1
然后再处理第二个元素7,将第八位置为1,,接着再处理第三个元素,一直到最后处理完所有的元素,将相应的位置为1,这时候的内存的Bit位的状态如 00111101
然后我们现在遍历一遍Bit区域,将该位是一的位的编号输出（2,3,4,5,7）,这样就达到了排序的目的。

python实现需要 bitarray 外部库，或者使用 bytearray,python并不擅长操作bit
...
# 数据分析Python技术栈
    ...
    graphic 企业级开源监控工具(数据绘图工具)，用于采集服务器实时信息并进行统计, 
    Graphite 自己本身并不收集具体的数据，这些数据收集的具体工作通常由第三方工具或插件完成. 其控制台前端项目 tessera
    logstash 日志收集和分析, 支持不同来源不同格式, 进行统一收集和分析处理


当你老了,回顾一生,就会发觉：什么时候出国读书、什么时候决定做第一份职业、何时选定了对象而恋爱、什么时候结婚,其实都是命运的巨变。
只是当时站在三岔路口,眼见风云千樯,你作出抉择的那一日,在日记上,相当沉闷和平凡,当时还以为是生命中普通的一天。---------《杀鹌鹑的少女》
...
gevent.joinall([g1,g2]) # 不加就不会运行协程，加了就一定要放在这里,缺点是阻塞后面的程序了
...
认知的不断进化和保持头脑的极度开放是解决影响合理决策的两个最大的障碍“自我意识和思维盲点”的最佳手段。
...
正如Linus说的：如果一个Bug被接受了，那个就不叫Bug，那叫一个Feature！

逻辑斯蒂增长模型(Logistic growth model)又称自我抑制性方程。
dx/dt=rx(1-x/k) 
x代表种群规模，t是时间，公式左边用dx除以dt就是种群增长速度，r代表最大生育率，k代表环境承载能力.
...
费雪方程式
MV=PT 
M表示一定时期流通中货币的平均数量；V表示一定时期单位货币的平均周转次数即货币流通速度；
P表示商品和劳务价格的加权平均数；T表示商品和劳务的交易数量。
说人话就是
M是指货币总量，V是指货币流通速度，P是指商品的平均价格，T是指总交易商品量。


P=MV/T
揭示出了价格的根本性真理，即：价格是成交额（货币）与商品的一种比值，它体现了付出货币方与付出商品方的力量对比关系。

货币中性论，即经济中的实物经济和货币经济的“二分法”思想。

弗里德曼指出，通货膨胀的真正原因在于货币供应增长率大于经济增长率，而货币量过多的原因都是出自于政府的错误政策和行为。是他们一手造成了通货膨胀，却还要嫁祸于人。通货膨胀所表现出来的普遍、持续物价上涨，就是由于货币发行过多所致。垄断了货币发行权的政府无疑有着不可推卸的责任。


将近两百名，高智商，接受过高等教育，而且经验丰富的工作人员，花费了八百多个日日夜夜（第三代开发历经两年多），投入数以百万美金计的资本——来娱乐我两个小时。
如果同样要占用我两个小时，老师只需要申请个教师，把去年的教案翻出来，改写一个PPT模板即可。
课堂总是输给游戏的真实原因，囊括了所有的论点论据，贯穿了一切的哲学原理，不管是“即时成就”，还是“小惊喜”，游戏的一切一切优势，每一分，都来自于无数的时间，金钱，和人力。

“游戏”只是被我取出来作为“娱乐产品”的代表而已，你可以把它替换成任何一个别的词，毫无关系。
